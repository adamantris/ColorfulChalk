		...
		mi.RegisterScriptMod(
			...
			.AddRule(
				...
				.With(
					...
					var proxy_node
					signal array_data(from, grid_pos, brush_size, color, canvas_id)
					signal array_ready(packets)
					var draw
			...
			.AddRule(
				new TransformationRuleBuilder()
				.Named("Assign our draw, connect a connection")
				.Do(Operation.Append)
				.Matching(TransformationPatternFactory.CreateFunctionDefinitionPattern("_ready", []))
				.With(
					"""

					draw = get_node("Viewport/TileMap/Node2D")
					connect("array_data", draw, "new_line")
			...
			.AddRule(
				new TransformationRuleBuilder()
				.Named("Remove most of vanilla _add_brush and send it ourselves")
				.Do(Operation.Append)
				.Matching(TransformationPatternFactory.CreateGdSnippetPattern(
					"""
				...
				.With(
					...
					emit_signal("array_data", _clamp_cell(from), _clamp_cell(grid_pos), brush_size, color, canvas_id)
					return
