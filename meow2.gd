...
func _ready ( ) : 
	...
	$ GridMap . clear ( ) 
	get_node ( "Viewport" ) . add_child ( load ( "res://mods/adamantris.ChromaChalk/scenes/proxy.gd" ) . new ( ) ) 
...
func _chalk_draw ( pos , size , color ) : 
	...
	if diff . length ( ) >= canvas_size && circular : 
...
func _add_brush ( grid_pos , type , from ) : 
	...
	var steps = 1 + MathFloor ( from . distance_to ( grid_pos ) / step_size ) 
	for s in steps : 
...
func _chalk_send_total ( id ) : 
	...
	if total_load . size ( ) >= 0 : 
		Network . _send_P2P_Packet ( { "type" : "chalk_packet" , "data" : total_load . duplicate ( ) , "canvas_id" : canvas_id } , TextStr ( id ) , 2 , Network . CHANNELS . CHALK ) 
...
func _clamp_cell ( pos , offset = 100 ) : 
	...
	final . x = LogicClamp ( final . x , - grid_offset , grid_offset * 2 ) 
	final . y = LogicClamp ( final . y , - grid_offset , grid_offset * 2 ) 
	return final 

func _screen_entered ( ) : 
	return 
	visible = True 
	$ Viewport / TileMap . visible = ! OptionsMenu . chalk_disabled 
func _screen_exited ( ) : 
	return 
	visible = False 
